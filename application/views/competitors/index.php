<div class="card card-stat p-3 mb-3">
	<form method="get" class="row g-2">
		...
		<div class="col-6 col-md-3">
			<a href="<?= base_url('competitors/create') ?>" class="btn btn-primary w-100"><i class="bi bi-plus-lg me-1"></i>Tambah Kompetitor</a>
		</div>
	</form>
</div>
...
<div class="card card-stat p-3">
	<div class="table-responsive">
		<table class="table align-middle">
			<thead><tr><th>No</th><th>Nama Kompetitor</th><th>Website</th><th>Status</th><th></th></tr></thead>
			<tbody>
			...
			<?php foreach ($competitors as $c): ?>
				<tr>
					...
					<td class="text-end text-nowrap">
						...
						<a href="<?= base_url('competitors/delete/' . $c['id']) ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Hapus kompetitor ini? Tindakan ini tidak dapat dibatalkan.')"><i class="bi bi-trash"></i></a>
					</td>
				</tr>
			<?php endforeach; ?>
	...
	<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mt-3">
		<div class="text-muted small">
			Menampilkan <?= $start ?>–<?= $end ?> dari <?= number_format($pagination['total'], 0, ',', '.') ?> kompetitor
		</div>
		<nav aria-label="Navigasi halaman kompetitor">
			<ul class="pagination pagination-sm mb-0 flex-wrap">
				<li class="page-item <?= $pagination['page'] <= 1 ? 'disabled' : '' ?>">
					<a class="page-link" href="<?= base_url('competitors') . '?' . http_build_query(array_merge($qs_base, array('page' => 1))) ?>">&laquo;&laquo;</a>
				</li>
				<li class="page-item <?= $pagination['page'] <= 1 ? 'disabled' : '' ?>">
					<a class="page-link" href="<?= base_url('competitors') . '?' . http_build_query(array_merge($qs_base, array('page' => $prev_page))) ?>">&laquo;</a>
				</li>
				<li class="page-item disabled"><span class="page-link">Halaman <?= $pagination['page'] ?> / <?= $pagination['total_pages'] ?></span></li>
				<li class="page-item <?= $pagination['page'] >= $pagination['total_pages'] ? 'disabled' : '' ?>">
					<a class="page-link" href="<?= base_url('competitors') . '?' . http_build_query(array_merge($qs_base, array('page' => $next_page))) ?>">&raquo;</a>
				</li>
				<li class="page-item <?= $pagination['page'] >= $pagination['total_pages'] ? 'disabled' : '' ?>">
					<a class="page-link" href="<?= base_url('competitors') . '?' . http_build_query(array_merge($qs_base, array('page' => $pagination['total_pages']))) ?>">&raquo;&raquo;</a>
				</li>
			</ul>
		</nav>
